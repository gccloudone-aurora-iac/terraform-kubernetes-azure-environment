...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_azure_resource_names"></a> [azure\_resource\_names](#module\_azure\_resource\_names) | ./modules/resource-names | n/a |
...
| <a name="input_cluster_admins_group_object_id"></a> [cluster\_admins\_group\_object\_id](#input\_cluster\_admins\_group\_object\_id) | The object ID of an existing Entra ID group to grant cluster admin. If null, the module creates the group. | `string` | `null` | no |
...
| <a name="input_cluster_linux_profile_ssh_key"></a> [cluster\_linux\_profile\_ssh\_key](#input\_cluster\_linux\_profile\_ssh\_key) | The SSH public key used to access the cluster nodes. A key is generated when null. | `string` | `null` | no |
| <a name="input_cluster_sku_tier"></a> [cluster\_sku\_tier](#input\_cluster\_sku\_tier) | The SKU tier of the AKS cluster, which sets its uptime SLA. Accepted values: Free, Standard, Premium | `string` | n/a | yes |
| <a name="input_cluster_support_plan"></a> [cluster\_support\_plan](#input\_cluster\_support\_plan) | The support plan for the AKS cluster. Accepted values: KubernetesOfficial, AKSLongTermSupport | `string` | n/a | yes |
...
| <a name="input_create_private_dns_zone_role"></a> [create\_private\_dns\_zone\_role](#input\_create\_private\_dns\_zone\_role) | Grant the cluster identity the Private DNS Zone Contributor role on the AKS private DNS zone. | `bool` | `true` | no |
...
| <a name="input_dns_servers"></a> [dns\_servers](#input\_dns\_servers) | The DNS servers to be used by the virtual network. Set to [] to use Azure DNS. | `list(string)` | <pre>[<br/>  "172.20.48.4",<br/>  "172.20.48.5"<br/>]</pre> | no |
...
| <a name="input_spn_object_ids"></a> [spn\_object\_ids](#input\_spn\_object\_ids) | The object IDs granted access to the created Azure resources. Defaults to the caller's own object ID when empty. | `list(string)` | n/a | yes |
| <a name="input_subnet_ids"></a> [subnet\_ids](#input\_subnet\_ids) | The subnet IDs within var.vnet\_id, keyed by subnet name. Required when var.vnet\_id is set. | `map(string)` | `null` | no |
...
| <a name="input_vnet_id"></a> [vnet\_id](#input\_vnet\_id) | The ID of an existing virtual network to deploy into. If null, the module creates one; if set, var.subnet\_ids is required. | `string` | `null` | no |
